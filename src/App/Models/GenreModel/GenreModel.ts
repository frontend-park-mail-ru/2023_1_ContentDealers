...
class GenreModel extends IModel {
    ...
    public async getSelectionsContent(id: number): Promise<IGrid> {
        const conf = Object.assign({}, config.api.selectionsById);
    ...
    public async getGenreContent(id: number): Promise<IGrid> {
        const conf = Object.assign({}, config.api.genresById);
        conf.url = conf.url.replace('{:id}', id.toString());
